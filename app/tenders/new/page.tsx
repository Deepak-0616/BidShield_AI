        ...
        <div className="max-w-4xl mx-auto space-y-6">
          ...
          {step === 1 && (
            <form onSubmit={startExtractionPipeline} className="bg-white p-8 rounded-2xl border border-slate-200 shadow-sm space-y-6 gold-accent-border">
              ...
              <div>
                <label className="block text-xs font-bold text-slate-700 uppercase mb-2">Tender Notice PDF Document</label>
                <input
                  type="file"
                  accept=".pdf"
                  ref={fileInputRef}
                ...
                <div
                  ...
                  className="border-2 border-dashed border-slate-300 rounded-xl p-8 text-center bg-slate-50 hover:bg-white hover:border-[#0B3A5B] transition cursor-pointer"
                >
                  <Upload className="w-8 h-8 text-[#0B3A5B] mx-auto mb-2" />
                  <p className="text-xs font-bold text-slate-700">
                    {selectedFile ? selectedFile.name : '01_Tender_GeM_IT_Infrastructure.pdf'}
                  </p>
                  <p className="text-[10px] text-slate-400 mt-1">
                    {selectedFile ? `${(selectedFile.size / 1024).toFixed(1)} KB • PDF Document Ready` : 'Click to select custom PDF or use synthetic GeM Tender Notice'}
                  </p>
                </div>
              </div>
